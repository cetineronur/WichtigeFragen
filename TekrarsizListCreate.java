package WichtigeFragen;


import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

public class TekrarsizListCreate {

    public static void main(String[] args) {
        /*
         * Verilen arraydeki tekrar eden sayilari, ilki haric silip, tekrarsiz
         * sayilardan olusan bir array haline getiren bir program yaziniz.
         *
         * INPUT : {1,2,2,3,1,4,2,5,6,8,7,5,9,1}
         * OUTPUT : [1,2,3,4,5,6,7,8,9]
         */
    	int[] array = {1,2,2,3,1,4,2,5,6,8,7,5,9,1};
    	List<Integer> tekrarsiz = new ArrayList<>();
    	
    	for (Integer s : array) {
			if (!tekrarsiz.contains(s)) {
				tekrarsiz.add(s);
			}
		}System.out.println("tekrarsizlarin list hali : "+tekrarsiz);
    	
		int[] arrTekrarsiz= new int[tekrarsiz.size()];
		
		for (int i = 0; i < arrTekrarsiz.length; i++) {
			arrTekrarsiz[i]=tekrarsiz.get(i);
		}System.out.println("tekrarsizlerin array hali: "+Arrays.toString(arrTekrarsiz));
		
    	/*for (int i = 0; i < array.length; i++) {
    		int flag=0;
			for (int j = 0; j <i; j++) {
				
				if (array[i] == array[j]) {
				flag = 1;
				break;
				}
			}
				if (flag == 0) {
				System.out.print(array[i] + " ");
				}
			}*/
    	}
   }